...
public class PaginationInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		if (obj instanceof PageQueryParameter) {
			PageQueryParameter pageQueryParameter = (PageQueryParameter) obj;

			if (pageQueryParameter.isPage()) {
				// 拦截到的prepare方法参数是一个Connection对象
				// 获取当前要执行的Sql语句，也就是我们直接在Mapper映射语句中写的Sql语句
				String sql = boundSql.getSql();
				// 获取分页Sql语句
				String pageSql = this.getPageSql(pageQueryParameter, sql);
	 ...
	 * @return Mysql数据库分页语句
	 */
	private String getMysqlPageSql(PageQueryParameter page, StringBuffer sqlBuffer) {
